...
function renderSetup() {
  ...
  for (let i = 0; i < state.playerCount; i++) {
    ...
    field.innerHTML =
      ...
      '<input type="text" id="name-' + i + '" value="' + escAttr(savedName || defaultName) + '" placeholder="' + escAttr(defaultName) + '" maxlength="20" autocomplete="new-password" />';
...
function renderGame() {
  ...
  renderOrder.forEach(function(i) {
    ...
    panel.innerHTML =
      ...
      '<input class="name-input" data-index="' + i + '" type="text" value="' + escAttr(player.name) + '" maxlength="20" autocomplete="new-password" aria-label="Player name" />' +
